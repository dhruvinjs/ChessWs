...
const GameControls = memo(({ 
  ...
  <div className="bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl shadow-xl w-full">
    <div className="flex items-center justify-center gap-4">
      ...
      >
        <span className="text-sm">{drawOfferSent ? "Draw Offer Sent" : "Offer Draw"}</span>
      </button>
    </div>
  </div>
));
...
function RoomChessPageComponent() {
  ...
  const isGameActive = useMemo(() => gameStatus === GameMessages.GAME_ACTIVE, [gameStatus]);
  ...
  const handleLeaveRoom = useCallback(async () => {
    const { leaveRoom, exitRoom, resignRoomGame, roomGameId, winner, gameStatus } = useGameStore.getState();
