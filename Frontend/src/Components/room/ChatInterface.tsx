...
const ChatInterfaceComponent = () => {
  ...
  const handleSend = useCallback(
    async (e: React.FormEvent) => {
      ...
      // Validation: need message and either roomId or roomGameId
      // User authentication is handled by WebSocket connection
      if (inputMessage.trim() === "" || (!roomId && !roomGameId)) {
        console.log("⚠️ Validation failed - not sending message", {
          hasMessage: inputMessage.trim() !== "",
          hasRoomId: !!roomId,
          hasRoomGameId: !!roomGameId,
        });
        return;
      }
      ...
      try {
        ...
        roomSocketManager.sendChatMessage(
          roomGameId || null,
          roomId || null,
          inputMessage.trim()
        );
    ...
    [inputMessage, roomId, roomGameId]
  );
    ...
    <div className="bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border border-slate-200 dark:border-slate-700 p-4 md:p-5 rounded-2xl shadow-xl flex flex-col w-full h-full">
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            <MessageRow
              key={index}
              message={msg}
              currentUserId={currentUserId || user?.id || 0}
      ...
      <form onSubmit={handleSend} className="mt-4 flex items-center gap-2">
        <ChatInput
          ...
          disabled={isSending || (!roomId && !roomGameId)}
        />

        <SendButton
          disabled={
            inputMessage.trim() === "" || isSending || (!roomId && !roomGameId)
          }
          loading={isSending}
          type="submit"
        />
