...
const CapturedPiecesComponent = ({ className = '' }: CapturedPiecesProps) => {
  ...
  if (capturedPieces.length === 0) {
    return (
      <div
        className={`bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl shadow-xl ${className}`}
      >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {myCapturedPieces.map((piece, index) => (
                ...
                >
                  <Piece piece={piece} className="w-6 h-6" />
                </div>
              ))}
            </div>
          </div>
