...
class RoomManager {
  ...
  async checkAndRestoreActiveGame(userId: number, userSocket: WebSocket) {
    try {
      ...
      if (room.status === "FULL") {
        ...
        console.log(`🔄 User ${userId} reconnecting to FULL room (isCreator: ${isCreator})`);
        userSocket.send(JSON.stringify({
          type: GameMessages.USER_HAS_JOINED,
          payload: {
            message: isCreator ? "Welcome back! Opponent is ready." : "Rejoined room successfully!",
  ...
  async roomJoined(createdById: number, opponentId: number) {
    try {
      ...
      console.log(`🔔 Notifying creator ${createdById} about opponent ${opponentId} joining`);
      ...
      if (createdBySocket) {
        ...
        console.log(`✅ USER_HAS_JOINED message sent to creator ${createdById}`);
      } else {
        console.log(`⚠️ Creator ${createdById} not connected to WebSocket yet - will sync on connection`);
      }
    } catch (error) {
  ...
  async messageHandlerForRoom(userId: number, userSocket: WebSocket) {
    userSocket.on("message", async (message: string) => {
      ...
      if (type === RoomMessages.INIT_ROOM_GAME) {
        ...
        await handleRoomDrawOffer(userId, userSocket, roomGameId, this.roomSocketManager);
      }
        ...
        await handleRoomDrawAcceptance(userId, userSocket, roomGameId, this.roomSocketManager);
      }
        ...
        await handleRoomDrawRejection(userId, userSocket, roomGameId, this.roomSocketManager);
      }
