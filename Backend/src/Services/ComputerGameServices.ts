...
export function validateComputerGamePayload(type:string,payload:any):string | null{
    if(!payload){
...
export async function getComputerGameState(gameId: number) {
  const gameExists = await redis.hGetAll(`computer-game:${gameId}`) as Record<string, string>;
  
  const movesRaw = await redis.lRange(`computer-game:${gameId}:moves`, 0, -1);
  const moves = movesRaw.map(m => JSON.parse(m));
    
  const capturedPieces = await redis.lRange(`computer-game:${gameId}:capturedPieces`, 0, -1);

  if (!gameExists || Object.keys(gameExists).length === 0 || gameExists.status === ComputerGameMessages.COMPUTER_GAME_OVER) {
...
export  async function handlePlayerMove(userId:number,userSocket:WebSocket,computerGameId:number,move:Move){
            ...
            if (board.isStalemate()) {
              await handleComputerGameDraw(userSocket, computerGameId, "Draw by stalemate");
              return;
            }
            else if(board.isInsufficientMaterial()){
            await handleComputerGameDraw(userSocket, computerGameId, "Draw due to insufficient material");
            return;
            }
            else if(board.isThreefoldRepetition()){
            await handleComputerGameDraw(userSocket,computerGameId, "Draw by threefold repetition");
            return;    
            }
            else if(board.isDraw()){
            await handleComputerGameDraw(userSocket, computerGameId, "Draw by 50-move rule");
            return;    
            }            
            if(board.isCheckmate()){
              ...
              const updatedCapturedPieces=await redis.lRange(`computer-game:${computerGameId}:capturedPieces`,0,-1)
              const movesRaw = await redis.lRange(`computer-game:${computerGameId}:moves`, 0, -1);
...
export async function handleComputerMove(userSocket:WebSocket,move:Move,computerGameId:number,userId:number) {
            ...
            if(board.isCheckmate()){
              ...
              const updatedMoveCount = await redis.hIncrBy(`computer-game:${computerGameId}`, "movesCount", 1);
...
export async function handleComputerGameDraw(userSocket: WebSocket, computerGameId: number, reason: string) {
  try {
    const gameState = await getComputerGameState(computerGameId);
    if (!gameState) {
      return;
...
async function saveComputerGameProgress(computerGameId: number, gameState: any) {
  try {
    const movesRaw = await redis.lRange(`computer-game:${computerGameId}:moves`, 0, -1);
    const moves = movesRaw.map(m => JSON.parse(m));
    const capturedPiecesRaw = await redis.lRange(`computer-game:${computerGameId}:capturedPieces`, 0, -1);
    ...
    console.log(`Computer Game ${computerGameId}: Progress saved to DB at move count ${moves.length}`);
  } catch (error) {
