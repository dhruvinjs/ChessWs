...
export async function getGameState(gameId: string) {
  const existingGame = (await redis.hGetAll(`game:${gameId}`)) as Record<
    string,
    string
  >;
...
export async function makeMove(
  ...
  if (
    (isWhiteTurn && gameState.user1 !== playerId) ||
  ...
  try {
    ...
    if (boardMove.captured) {
      ...
      await redis.rPush(`game:${gameId}:capturedPieces`, capturedPiece);
  ...
  const currentPlayerSocket =
    playerId === gameState.user1 ? user1Socket : user2Socket;
  const opponentSocket =
    playerId === gameState.user1 ? user2Socket : user1Socket;
...
export async function reconnectPlayer(
  ...
  if (game.status === GameMessages.DISCONNECTED) {
    const timeElapsed =
      Date.now() - Number(await redis.hGet(`game:${gameId}`, 'timestamp'));
...
export async function offerDraw(
  ...
  const draw_offer_recieverId =
    game.user1 === playerId ? game.user2 : game.user1;
...
export async function rejectDraw(
  playerId: string,
  gameId: string,
  offerRejecterSocket: WebSocket,
  socketMap: Map<string, WebSocket>
) {
  const game = await getGameState(gameId);
  if (!game || game.gameEnded) {
    offerRejecterSocket.send(
      JSON.stringify({
        type: GameMessages.GAME_NOT_FOUND,
        payload: { message: 'The Game You Are looking for is over' },
      })
